...
var PopoverContent = (function (_super) {
    ...
    function PopoverContent(props) {
        ...
        _this.adjustPosition = function () {
            ...
            switch (position) {
                case 'topCenter':
                    newPositionStyle = __assign({}, positionStyle, { left: triggerRect.left + (triggerRect.width / 2) - (contentRect.width / 2) + offsetX, top: window.pageYOffset + triggerRect.top - contentRect.height - 10 + offsetY });
                    break;
                case 'bottomCenter':
                    newPositionStyle = __assign({}, positionStyle, { left: triggerRect.left + (triggerRect.width / 2) - (contentRect.width / 2) + offsetX, top: window.pageYOffset + triggerRect.top + triggerRect.height + offsetY });
                    break;
                case 'bottomRight':
                    newPositionStyle = __assign({}, positionStyle, { left: triggerRect.left - contentRect.width + offsetX, top: window.pageYOffset + triggerRect.top + triggerRect.height + offsetY });
                    break;
                case 'bottomLeft':
                default:
                    newPositionStyle = __assign({}, positionStyle, { left: triggerRect.left + offsetX, top: window.pageYOffset + triggerRect.top + triggerRect.height + offsetY });
            }
